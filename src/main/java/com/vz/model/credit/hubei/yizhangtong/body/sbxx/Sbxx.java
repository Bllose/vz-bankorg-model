package com.vz.model.credit.hubei.yizhangtong.body.sbxx;


public class Sbxx {

    private String zsxmmc; // 征收项目名称 个人所得税
    private String zsxm_dm; // 征收项目代码 10106
    private String zsfsmc; // 查账征收
    private String zsfs_dm; // 100
    private String ysbqx; // 2019-01-15
    private String skssqq; // 2018-10-01
    private String skssqz; // 2018-12-31
    private String ysx; // 0
    private String ynse; // 0
    private String yjse; // 0
    private String jmse; // 0
    private String ybtse; // 0
    private String gzlx_dm_1; // 1
    private String sbqx; // 2019-01-15

    public String getZsxmmc() {
        return zsxmmc;
    }

    public void setZsxmmc(String zsxmmc) {
        this.zsxmmc = zsxmmc;
    }

    public String getZsxm_dm() {
        return zsxm_dm;
    }

    public void setZsxm_dm(String zsxm_dm) {
        this.zsxm_dm = zsxm_dm;
    }

    public String getZsfsmc() {
        return zsfsmc;
    }

    public void setZsfsmc(String zsfsmc) {
        this.zsfsmc = zsfsmc;
    }

    public String getZsfs_dm() {
        return zsfs_dm;
    }

    public void setZsfs_dm(String zsfs_dm) {
        this.zsfs_dm = zsfs_dm;
    }

    public String getYsbqx() {
        return ysbqx;
    }

    public void setYsbqx(String ysbqx) {
        this.ysbqx = ysbqx;
    }

    public String getSkssqq() {
        return skssqq;
    }

    public void setSkssqq(String skssqq) {
        this.skssqq = skssqq;
    }

    public String getSkssqz() {
        return skssqz;
    }

    public void setSkssqz(String skssqz) {
        this.skssqz = skssqz;
    }

    public String getYsx() {
        return ysx;
    }

    public void setYsx(String ysx) {
        this.ysx = ysx;
    }

    public String getYnse() {
        return ynse;
    }

    public void setYnse(String ynse) {
        this.ynse = ynse;
    }

    public String getYjse() {
        return yjse;
    }

    public void setYjse(String yjse) {
        this.yjse = yjse;
    }

    public String getJmse() {
        return jmse;
    }

    public void setJmse(String jmse) {
        this.jmse = jmse;
    }

    public String getYbtse() {
        return ybtse;
    }

    public void setYbtse(String ybtse) {
        this.ybtse = ybtse;
    }

    public String getGzlx_dm_1() {
        return gzlx_dm_1;
    }

    public void setGzlx_dm_1(String gzlx_dm_1) {
        this.gzlx_dm_1 = gzlx_dm_1;
    }

    public String getSbqx() {
        return sbqx;
    }

    public void setSbqx(String sbqx) {
        this.sbqx = sbqx;
    }
}
